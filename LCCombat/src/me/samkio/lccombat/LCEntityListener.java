		...
		if (event instanceof EntityDamageByEntityEvent) {
			if (((EntityDamageByEntityEvent) event).getDamager() instanceof Player
				...
				if (!(event instanceof EntityDamageByProjectileEvent)) {
					...
					plugin.entListener.onEDamageByE((EntityDamageByEntityEvent) event);
					return;
				}
			}
		}
	...
	public void onEDamageByE(EntityDamageByEntityEvent event){
		...
		int level = LevelFunctions.getLevel(player, plugin.thisPlug);
			if (level < plugin.LCConfiguration.IronSword && iih == 267) {
				...
				LevelFunctions.addExp(player, plugin.thisPlug, plugin.LCConfiguration.ExpPerDamage * event.getDamage());
