...
public class Tools {
	...
	public String getIndexBar(Player p){
		...
		for (Plugin p1 : plugin.LevelIndexes.keySet()) {
			if(one && plugin.Permissions.hasLevelExp(p, p1)) str = str + "/";
			if(plugin.Permissions.hasLevelExp(p, p1)) {str = str + plugin.LevelIndexes.get(p1);
			one = true;
			}
		}
		str = str + "]";
	...
	public Plugin getPluginFromName(String s){
		for(Plugin p:plugin.LevelNames.keySet()){
			if(plugin.LevelNames.get(p).equalsIgnoreCase(s)) return p;
		}
		return null;
	}
}
